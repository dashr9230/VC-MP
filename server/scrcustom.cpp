...
{
	float fGravity = (params[1] / 100.0f) * 0.008f;
...
static cell AMX_NATIVE_CALL n_Taxiboostjump(AMX *amx, cell *params)
{
	pNetGame->m_GameSettings.bTaxiBoostJump = (BYTE)params[1] == 1;
	pNetGame->SendGameSettingsToAll();
	return 1;
}
...
static cell AMX_NATIVE_CALL n_GetPlayerScore(AMX *amx, cell *params)
{
	BYTE bytePlayerID = (BYTE)params[1];
	if (pNetGame->GetPlayerPool()->GetSlotState(bytePlayerID)) {
		return pNetGame->GetPlayerPool()->GetScore(bytePlayerID);
	}
	return 0;
}
